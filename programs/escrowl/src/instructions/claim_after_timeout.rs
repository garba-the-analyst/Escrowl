//! claim_after_timeout: seller claims after window with no dispute.
...
pub struct ClaimAfterTimeout<'info> {
    ...
    #[account(mut, has_one = seller, has_one = vault, has_one = mint)]
...
pub fn handler(ctx: Context<ClaimAfterTimeout>, index: u8) -> Result<()> {
    // I9: exits never blocked by pause.
    require!(
    ...
    // Boundary: claim allowed at exactly deadline (now >= deadline).
    require!(now >= deadline, EscrowError::WindowNotElapsed);
